...
%% Plotting the reference line
  ...
  loglog((step_size),100*step_size.^2,'--k')
 %% Title
...
%% Labelling the axis
xlabel('step size \tau')
ylabel('Maximal Error in $\textbf{P}=(0,0,0)$','Interpreter','Latex')
...
hold off
  legend(strcat('$h=1/2$'), '$\mathcal O(\tau^2)$' ,'location','southeast');  
...
%     strcat('$N$ = ',num2str(N_s(7))),strcat('$N$ = ',num2str(N_s(7))),'$\mathcal O(h)$','$\mathcal O(h^{3/2})$'},'Interpreter','Latex','location','southeast');
 l=legend({strcat('$N = 32$'),'$\mathcal O(h)$','$\mathcal O(h^{3/2})$'},'Interpreter','Latex','location','southeast');
