...
#        filename = 'data/thesis_nonlinear/density_two_cubes_h_'+str(np.round(h,3)) +'_N_'+str(N)+'_m_'+str(m)+'_a_'+str(alpha)+ '_I.npy'
...
for space_index in range(am_space):
    for time_index in range(am_time):
        ...
        tau = T*1.0/N
        gridfilename = 'data/grids/two_cubes_h_'+str(np.round(h,3))+'.npy'
        filename = 'data/thesis_nonlinear/density_two_cubes_h_'+str(np.round(h,3)) +'_N_'+str(N)+'_m_'+str(m)+'_a_'+str(alpha)+'_T_'+str(T)+ '_I.npy'
        if os.path.isfile(filename):
            print("File "+filename+" already computed, jumped.")
            continue
        rk = RKMethod("RadauIIA-"+str(m),tau)
        start = time.time()
        try:
            sol_newt = compute_densities(alpha,N,gridfilename,T,rk,debug_mode=False)
        except:
            print("Computation of "+filename+" failed, continue.")
            continue
        norm_newt =sum(np.linalg.norm(sol_newt[:,::m],axis = 0))
