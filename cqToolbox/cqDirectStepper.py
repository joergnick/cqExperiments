...
class AbstractIntegratorDirect:
    ...
    def integrate(self,T,N,method = "RadauIIA-2",tolsolver = 10**(-10),max_evals_saved=100000,factor_laplace_evaluations = 2,debug_mode=False,same_rho = False,same_L = False):
        ...
        for j in range(0,N):
            ...
            #    history = sol[:,1:Nloc]
            globalconvHist = np.real(self.tdForward.apply_RKconvol(history,T,cutoff = 10**(-15),method = method,factor_laplace_evaluations=factor_laplace_evaluations,prolonge_by=0,show_progress=False))
