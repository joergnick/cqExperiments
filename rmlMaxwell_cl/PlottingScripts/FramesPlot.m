...
load('../data/sim_data_rml.mat')
X_mesh(:,:)=plot_grid(1,:,:);
Y_mesh(:,:)=plot_grid(2,:,:);
n_grid=sqrt(length(u_ges(:,1))) ; 

[scatterer ] = create_cube_edges(X_mesh,Y_mesh);
%scatterer = scatterer * 0;
% figure(11)
 %spy(scatterer)
...
%frames=[800,1000,1200,1400,1600,1800];

frames = zeros(9);
for j=1:9
    frames(j) = 524+131+131*j;
end
...
%    colormap(mymap);
  u_long=u_ges(:,j);
   ...
   % view(180,0)
    ...
    startp2=1;
    %endp2=n_grid2;
      title(strcat('t= ',num2str(8/2096*frames(n))),'interpreter','latex')
